...
CRenderer::CRenderer()
{
	...
	m_pZBufferSurface = nullptr;		// Zバッファ用サーフェイスのポインタ
	m_pOrgSurface = nullptr;			// 元の描画用サーフェイスのポインタ
	m_pOrgZBuffeSurface = nullptr;		// 元のZバッファ用サーフェイスのポインタ
}
...
HRESULT CRenderer::Init(HWND hWnd, bool bWindow)
{
	...
	// レンダーテクスチャの設定
	D3DXCreateTexture(m_pD3DDevice, SCREEN_WIDTH, SCREEN_HEIGHT, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &m_pRenderTexture);

	// 描画サーフェイスの設定
	m_pRenderTexture->GetSurfaceLevel(0, &m_pDrowSurface);

	// Zバッファサーフェイスの設定
	m_pD3DDevice->CreateDepthStencilSurface(SCREEN_WIDTH, SCREEN_HEIGHT, D3DFMT_FORCE_DWORD, D3DMULTISAMPLE_NONE, 0, FALSE, &m_pZBufferSurface, NULL);

	// 元のサーフェイスの保存
	m_pD3DDevice->GetRenderTarget(0, &m_pOrgSurface);
	m_pD3DDevice->GetDepthStencilSurface(&m_pOrgZBuffeSurface);
...
void CRenderer::Uninit()
{
	...
	// 元のZバッファ用サーフェイスの破棄
	if (m_pOrgZBuffeSurface != nullptr)
	{
		m_pOrgZBuffeSurface->Release();
		m_pOrgZBuffeSurface = nullptr;
	}
}
...
void CRenderer::Draw()
{
	...
	// 描画サーフェイスの設定
	m_pD3DDevice->SetRenderTarget(0, m_pDrowSurface);
	...
	// Zバッファサーフェイスの設定
	m_pD3DDevice->SetDepthStencilSurface(m_pOrgZBuffeSurface);
